    ...
    <table>
      ...
      <tbody>
        <tr v-for="book in paginatedBooks" :key="book._id">
          <td style="text-align: center;">{{ book.MASACH || 'Không có mã' }}</td>
          <td>{{ book.TENSACH || 'Không có tên' }}</td>
          <td style="text-align: center;">{{ book.NGUONGOC_TACGIA || 'Không có tên' }}</td>
          <td style="text-align: center;">{{ getNXBName(book.MANXB) }}</td>
